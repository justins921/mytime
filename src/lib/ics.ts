  ...
  for (let i = 1; i < blocks.length; i++) {
    ...
    for (const line of lines) {
      if (line.startsWith("UID:")) {
        ...
        const { value, isDate } = parseDTValue(line);
        dtstart = value;
        if (isDate) allDay = true;
      } else if (line.startsWith("DTEND")) {
        const { value } = parseDTValue(line);
        dtend = value;
      }
    }
    ...
    const start = parseICSDate(dtstart, allDay);
    const end = dtend ? parseICSDate(dtend, allDay) : new Date(start.getTime() + 60 * 60 * 1000);
...
function parseDTValue(line: string): { value: string; isDate: boolean } {
  // Handle formats like:
  // DTSTART:20260212T090000Z
  // DTSTART;VALUE=DATE:20260212
  // DTSTART;TZID=America/Chicago:20260212T090000
  const colonIdx = line.indexOf(":");
  if (colonIdx === -1) return { value: "", isDate: false };
